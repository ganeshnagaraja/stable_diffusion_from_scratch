...
class VAE_AttentionBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        # (batch_size, features, height * width) --> (bath_size, height * width, features)
        x = x.transpose(-1, -2)

        # (batch_size, height * width, features)
        x = self.attention(x)

        # Going back to original shape after calculating attention
        # (batch_size, height * width, features) --> (bath_size, features, height * width)
        x = x.transpose(-1, -2)
